export async function onRequest(context) {
  ...
  const owner = (env.GITHUB_OWNER || "aoz-jcf-1165").trim(); // 必要なら env化
  const repo  = (env.GITHUB_REPO  || "TSC-event-survey-web-2025.12").trim(); // 必要なら env化
  ...
  const issueBody = [
    ...
    "```json",
    JSON.stringify(
      { timestamp: stamp, language, player_name, Q2_time, Q3_time, Q4_day },
      null,
      2
    ),
    "```",
  ...
  // --- GitHub API呼び出し（失敗時も必ずJSONで理由を返す） ---
  ...
  return json(
    200,
    {
      ...
      message: "Submitted.",
      issue: ghJson
        ? { number: ghJson.number, url: ghJson.html_url }
        : null,
      requestId,
      time: now,
      ray,
...
function json(status, obj, headers = {}) {
  // status=204 & obj=null でも JSON を返さないようにする
